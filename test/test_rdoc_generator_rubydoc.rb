require 'rubygems'
require 'rdoc/generator/rubydoc'
require 'minitest/autorun'
require 'tmpdir'
require 'nokogiri'

class TestRDocGeneratorRubydoc < MiniTest::Unit::TestCase

  def setup

    @srcdir = File.expand_path(File.join(File.dirname(__FILE__), 'data'))

    @tmpdir = Dir.mktmpdir('rdoc_rubydoc_')
    @delete = true
    @dirnum = 0
  ...
  def test_first_page
    ...
    assert_main 'classes/RDocRubydoc/Documented.html'
    ...
    assert_main 'classes/RDocRubydocWithContent.html'

    # - The first class or module.

    make_doc %w(no_content.rb)
    assert_main 'classes/RDocRubydoc.html'
...
private
  ...
  def check_call_seq(name, call_seq, html)
    ...
    m.call_seq = call_seq
    assert_equal html, rubydoc_gen.decorated_call_seq(m, 'c')
  end

  def rubydoc_gen
    @rubydoc_gen ||= begin
      options = Struct.new(:rubydoc_options, :line_numbers, :template).new
      options.template = 'rubydoc'
      options.rubydoc_options = {}
      RDoc::Generator::Rubydoc.new(options)
    end
  end
